...
bool SQLiteResultSet::next() {
  ...
  default:
    ...
    // sqlite3_step()
    _statement.reset();
    throw SQLiteSqlError(_statement.getDb(), "sqlite3_step() failed",
                         _statement.getSQL());
  }
...
void SQLiteResultSet::checkRowAndColumn(const int columnIndex) const {
  ...
  if (columnIndex >= _numColumns) {
    ...
    msg << "Column index " << columnIndex << " past last result column index "
        << _numColumns - 1;
...
void SQLiteResultSet::getString(const int columnIndex, std::string &out) const {
  ...
  const unsigned char *result =
      sqlite3_column_text(_statement.handle(), columnIndex);
