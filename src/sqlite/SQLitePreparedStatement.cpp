...
sqlite3_stmt *init_statement(dbc::SQLiteConnection &db,
...
const CountProxy &SQLitePreparedStatement::doExecuteUpdate() {
  ...
  if (ret != SQLITE_DONE) {
    ...
    throw SQLiteSqlError(
        _db, "sqlite3_step() must return SQLITE_DONE in executeUpdate()",
        getSQL());
  }
...
void SQLitePreparedStatement::setString(const int index,
                                        const std::string &value) {
  _param_tracker.setParameter(index);
  // Note that SQLITE_TRANSIENT makes copy of the data.
  // This may be expensive if it is large.

  int ret = sqlite3_bind_text(_statement.get(), index, value.c_str(), -1,
                              SQLITE_TRANSIENT);
  THROW_IF_SET_STMT_NOT_OK(
